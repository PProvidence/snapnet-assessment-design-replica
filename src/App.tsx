...
export default function App() {
  return (
    ...
    <div className="min-h-screen flex items-center justify-center bg-bg-primary p-0 2xl:p-10 transition-colors duration-300">
      <img 
        src={outerDecor} alt="" 
        className="absolute top-0 right-0 w-[40%] max-w-[710px] pointer-events-none hidden dark:hidden 2xl:block z-0" 
      />
      {/*Illustration (Visible in Dark Mode)  */}
      <img 
        src={outerDarkDecor}  alt="" 
        className="absolute top-0 right-0 w-[40%] max-w-[710px] pointer-events-none hidden dark:block 2xl:block z-0" 
      />
      {/* MAIN CARD */}
      <div className="relative w-full max-w-[1440px] min-h-screen 2xl:min-h-[900px] 2xl:h-auto 2xl:aspect-[1440/900] bg-bg-card 2xl:rounded-[20px] shadow-2xl overflow-hidden flex flex-col transition-colors duration-300">
        ...
        <div className="absolute inset-0 pointer-events-none z-0">
          <img src={decorLeft} alt="" className="absolute bottom-0 left-0 w-[40%] max-w-[600px]" />
          <img src={decorRight} alt="" className="absolute top-0 right-0 w-[35%] max-w-[500px]" />
        </div>
        ...
        <div className="relative z-10 flex flex-col h-full px-5 sm:px-6 md:px-16 lg:px-24 py-8">
          ...
          <nav className="flex justify-between items-center w-full mb-8 lg:mb-12">
            ...
            <div className="flex gap-2.5 sm:gap-6">
              <TbBrandTwitter className="size-5 md:size-6 text-icon-header hover:text-brand-green cursor-pointer transition-colors" />
              <TiSocialFacebookCircular className="size-5 md:size-6 text-icon-header hover:text-brand-green cursor-pointer transition-colors" />
              <TbBrandMedium className="size-5 md:size-6 text-icon-header hover:text-brand-green cursor-pointer transition-colors" />
            </div>

            {/* Logo Section */}
            <div className="flex items-center gap-2 sm:gap-3 absolute left-1/2 -translate-x-1/2">
              <img src={logoIcon} alt="Finance Logo" className="h-6 sm:h-10 w-auto" />
